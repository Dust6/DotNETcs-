...
namespace Commmon
{
    ...
    {
        private static string connString = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source={0};Extended Properties=Excel 12.0"; //版本引擎
        /// <summary>
        ...
        public static OleDbDataReader GetReader(string sql)
        {
            ...
            {
                conn.Open();
                OleDbDataReader objReader = cmd.ExecuteReader(CommandBehavior.CloseConnection);
                return objReader;
            }
            catch (Exception ex)
            {
        ...
        public static DataSet GetDataSet(string sql)
        {
            ...
            OleDbDataAdapter dater = new OleDbDataAdapter(cmd);//创建适配器对象
            DataSet dset = new DataSet();//创建内存数据集
            try
            {
                con.Open();
                dater.Fill(dset);
